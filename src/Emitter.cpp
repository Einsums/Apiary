...
// emit_method needs them for the APIARY_INSTANTIATE_MEMBER_AS
...
void emit_method(llvm::raw_string_ostream &os, std::string const &class_qual, BoundMethod const &m,
    ...
    // Member-template fan-out (APIARY_INSTANTIATE_MEMBER_AS):
...
// non-templated classes; used by APIARY_VARIADIC_FROM expansion
...
// APIARY_NO_BASES on every class with internal-only inheritance.
void emit_class_body(llvm::raw_string_ostream &os, BoundClass const &cls, std::string const &concrete_type, std::string const &py_name,
    ...
    for (auto const &ctor : cls.ctors) {
        ...
        // APIARY_VARIADIC_FROM expansion: replace the (assumed-last)
...
// APIARY_TEMPLATE_KWARGS). The lambda takes the function's normal
